interface HeaderProps {
    data: any;
}

const PageHeaderWithBg: React.FC<HeaderProps> = ({ data }) => {
    const { heroTitle, heroDescription, heroBackgroudImageUrl } =
        data?.nodeByUri.hero;
    return (
        <div
            className="flex justify-center p-6 md:p-10 2xl:p-8 relative bg-no-repeat bg-center bg-cover"
            style={{
                backgroundImage: `url(${heroBackgroudImageUrl.heroBackgroudImageUrl.node.sourceUrl})`,
            }}
        >
            <div className="absolute top-0 start-0 bg-black w-full h-full opacity-50 transition-opacity duration-500 group-hover:opacity-80" />
            <div className="w-full flex items-center justify-center relative z-10 py-10 md:py-14 lg:py-20 xl:py-24 2xl:py-32">
                <h2 className="text-xl md:text-2xl lg:text-3xl font-bold theme-text-heading-with-image-bg text-center">
                    <span className="font-satisfy block font-normal mb-3">
                        {heroTitle}
                    </span>
                    {heroDescription}
                </h2>
            </div>
        </div>
    );
};
export default PageHeaderWithBg;
