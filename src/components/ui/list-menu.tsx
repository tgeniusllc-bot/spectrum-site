...
const ListMenu = ({
    ...
    return (
        <li className={cn(!hasMegaMenu ? "group relative" : "")}>
            <Link
                href={data.path}
                className="flex items-center py-2 ps-5 xl:ps-7 pe-3 xl:pe-3.5 theme-navbar-submenu-text hover:bg-theme hover:bg-opacity-45 "
            >
                {data.icon && (
                    <span className="inline-flex me-2">{data.icon}</span>
                )}
                {t(data.label)}
                {data.subMenu && (
                    <span className="text-sm mt-0.5 shrink-0 ml-auto">
                        <IoIosArrowForward className="theme-navbar-submenu-text" />
                    </span>
                )}
            </Link>
            {hasSubMenu && (
